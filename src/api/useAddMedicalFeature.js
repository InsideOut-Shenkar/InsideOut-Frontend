...
const useAddMedicalFeature = () => {
  const addMedicalFeature = async (medicalDataID, newFeatures) => {
    try {
      const token = await getCurrentUserToken();

      const body = {
        medical_info: newFeatures
      };
      console.log(body);
  ...
  return { addMedicalFeature };
};

export default useAddMedicalFeature;
